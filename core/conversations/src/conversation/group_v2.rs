...
pub struct GroupV2Convo {
    ...
    /// Joiners WE invited, as `(member_id, routing_id)`: the de-mls member id
    /// (the joiner's leaf credential content, read from its key package) paired
    /// with the signer routing id its welcome is delivered to.
    ...
    S: ExternalServices,
{
    ...
    fn add_member(
        ...
        // signer routing ids; the de-mls member id must equal the joiner's MLS
        // leaf credential content (de-mls matches members by credential), so it
        // is read from the fetched key package rather than assumed.
...
impl GroupV2Convo {
    fn after_op<S: ExternalServices>(
        ...
        //    now). The welcome travels to the joiner's signer routing id, not
        //    its de-mls member id.
        for evt in &events {
