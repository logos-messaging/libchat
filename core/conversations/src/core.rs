    ...
    CS: ChatStore + 'static,
{
    ...
    fn assemble(
        ...
        // InboxV2 rendezvous is signer-scoped: the routing id is the hex of the
        // signer's verifying key — the same string the account → device
...
impl<'a, S: ExternalServices + 'static> Core<S> {
    ...
    /// The signer routing id this core receives InboxV2 invites under — the hex
    /// of the signer's verifying key.
